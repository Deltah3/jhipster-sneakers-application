...
public class CommandeRepositoryWithBagRelationshipsImpl implements CommandeRepositoryWithBagRelationships {
    ...
    public Optional<Commande> fetchBagRelationships(Optional<Commande> commande) {
        return commande.map(this::fetchSneakersses);
    }
    ...
    public List<Commande> fetchBagRelationships(List<Commande> commandes) {
        return Optional.of(commandes).map(this::fetchSneakersses).orElse(Collections.emptyList());
    }

    Commande fetchSneakersses(Commande result) {
        return entityManager
            .createQuery(
                "select commande from Commande commande left join fetch commande.sneakersses where commande.id = :id",
                Commande.class
            )
    ...
    List<Commande> fetchSneakersses(List<Commande> commandes) {
        ...
        List<Commande> result = entityManager
            .createQuery(
                "select commande from Commande commande left join fetch commande.sneakersses where commande in :commandes",
                Commande.class
            )
